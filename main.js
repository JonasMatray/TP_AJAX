...
function searchGeneral(searchName, searchAccel) {
    ...
    for (let line of linesCars) {
        ...
        if (!(line.id === '')) {
            let lineName = line.firstChild.textContent.toLowerCase();
            let lineAccel = parseFloat(line.children[5].textContent);
            if (!lineName.match(searchName.value.toLowerCase()) || (!(lineAccel < searchAccel.value) && !(searchAccel.value.toString() === ''))) {
